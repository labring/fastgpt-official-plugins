...
export async function handler(
  ...
  const contentSourceUrls = contentSourceUrl
    ? parseStringOrArray(contentSourceUrl)
    : undefined;
  const needOpenComments = Array.isArray(needOpenComment)
    ? needOpenComment
    : [needOpenComment];
  ...
  for (let i = 0; i < articleCount; i++) {
    try {
      ...
      const currentCoverImage =
        coverImages.length === 1 ? coverImages[0] : coverImages[i];
      const currentTitle = titles
        ? titles.length === 1
          ? titles[0]
          : titles[i]
        : undefined;
      const currentAuthor = authors
        ? authors.length === 1
          ? authors[0]
          : authors[i]
        : undefined;
      const currentDigest = digests
        ? digests.length === 1
          ? digests[0]
          : digests[i]
        : undefined;
      ...
      const currentNeedOpenComment =
        needOpenComments.length === 1
          ? needOpenComments[0]
          : needOpenComments[i];
      const currentOnlyFansCanComment =
        onlyFansCanComments.length === 1
          ? onlyFansCanComments[0]
          : onlyFansCanComments[i];
...
async function uploadImageToWeChat(
  ...
  try {
    // 使用统一的图片下载方法
    const imageBlob = await downloadImageFromUrl(
      imageUrl,
      "image.jpg",
      "image/jpeg",
    );
...
async function processCoverImage(
  ...
  try {
    ...
    if ("errcode" in result && result.errcode !== 0) {
      const errorMsg = (result as any).errmsg || "未知错误";
      throw new Error(
        `上传封面图失败: ${errorMsg} (错误码: ${result.errcode})`,
      );
    }
