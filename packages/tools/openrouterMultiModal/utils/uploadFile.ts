...
export type UploadContext = {
  invoke?: {
    uploadFile(input: any): Promise<[UploadResult | null, unknown]>;
  };
};
...
export async function uploadFile(input: UploadInput, ctx?: UploadContext) {
  ...
  if (ctx?.invoke?.uploadFile) {
    ...
    const accessUrl = (result.accessUrl ?? result.accessURL) as string | undefined;
  ...
  return { accessUrl: dataUrl, accessURL: dataUrl, fileName: file.fileName, contentType: file.contentType };
}
...
function getUploadErrorMessage(error: unknown) {
  ...
  if (typeof error === 'object' && 'reason' in error) {
    const reason = (error as { reason?: { 'zh-CN'?: string; en?: string } }).reason;
...
async function normalizeInput(input: UploadInput) {
  ...
  if (input.base64) {
    ...
    const contentType = input.contentType ?? match?.[1] ?? guessContentType(fileName);
  ...
  if (input.url) {
    ...
    return {
      ...
      fileName,
      contentType: input.contentType ?? response.headers.get('content-type') ?? guessContentType(fileName)
    };
  }
