...
- **0.0.2** - 新增 SSL 配置，支持为数据库连接启用 SSL/TLS。
...
所有数据库连接配置都提供 **启用 SSL** 选项，默认关闭。开启后各数据库使用对应的加密连接方式：

- **MySQL** - 使用 TLS 连接
- **PostgreSQL** - 使用 SSL 连接
- **Microsoft SQL Server** - 启用加密连接
- **ClickHouse** - 将 HTTP 连接切换为 HTTPS
- **Oracle** - 启用服务端证书 DN 匹配；使用 TLS 时，`connectString` 还需要配置 `tcps` 连接描述和对应的 wallet/证书
