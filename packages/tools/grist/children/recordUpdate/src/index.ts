...
export const InputType = z.object({
  ...
  recordId: z.number().int().positive("Record ID is required"),
...
export async function tool({
  ...
  try {
    ...
    const body: GristRecordsUpdateRequest = {
      records: [{ id: recordId, fields: fieldsObject }],
    };

    const response = await client.patch<GristRecordsResponse>(
