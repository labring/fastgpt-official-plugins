import { z } from "zod";
import {
  createDataForB2BClient,
  handleDataForB2BError,
  parseJsonParam,
} from "../../client";
...
export async function tool(
  ...
  try {
    const { apiKey, query, category, session_id, max_results, enrich_live } =
      props;
    const answers = parseJsonParam(props.answers, "answers") as
    ...
    const response = await client.post<ReasoningResponse>(
      "/search/reasoning",
      body,
    );
