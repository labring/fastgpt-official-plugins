...
export function createDataForB2BClient(apiKey: string): AxiosInstance {
  if (!apiKey || typeof apiKey !== "string") {
    throw new Error(
      "DataForB2B API key is required. Get it from app.dataforb2b.ai.",
    );
  }
  return axios.create({
...
export function buildSlotCondition(
  ...
  if (op === "between") {
    ...
    if (parts.length < 2) {
      throw new Error(
        `Operator 'between' on '${col}' needs two comma-separated values, e.g. 3,7`,
      );
    }
    return {
      column: col,
...
export function finalizeFilters(
  ...
  const group: FilterGroup | undefined =
    conditions.length > 0
      ? { op: op as "and" | "or", conditions: [...conditions] }
      : undefined;
  ...
  if (advanced) {
    ...
    if (
      ...
      advanced !== null &&
      "conditions" in (advanced as object)
    ) {
