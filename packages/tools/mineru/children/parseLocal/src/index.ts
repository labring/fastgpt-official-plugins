...
const ResultItemType = z.object({
  ...
  contentList: z.array(z.any()).optional(),
});
...
export async function tool(
  ...
  const parsedFiles: ResultItemType[] = [];
  ...
  for (const [parsedFilename, result_item] of Object.entries(
    ...
    if (result_item.content_list) {
      try {
        item.contentList = JSON.parse(result_item.content_list);
      } catch {
    ...
    if (result_item.md_content) {
      item.markdown = replaceImageUrl(result_item.md_content, images);
    }
