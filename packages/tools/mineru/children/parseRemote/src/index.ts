...
export const OutputType = z.object({
  result: z.string(),
  files: z.array(OutputResultSchemaItem),
});
...
async function extractResult(
  ...
  const checkResult = async () => {
    ...
    if (data.code !== 0) {
      return Promise.reject(
        data.msg || ErrorCodeMap[data.code] || "Unknown error",
      );
    }
...
export async function tool(
  ...
  const files = await extractResult(batchId, innerProps, ctx);
