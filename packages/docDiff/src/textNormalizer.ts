...
function processSpaces(
  ...
  const { removeTextSpaces, enableIntelligentSpaces, removeExtraSpaces } =
    options;
...
function processTableRow(
  ...
  if (removeTextSpaces) {
    ...
    return line
      .split("|")
      .map((cell) => {
        // biome-ignore lint/suspicious/noControlCharactersInRegex: Used to detect non-ASCII characters
...
function convertFullWidthToHalfWidth(text: string): string {
  ...
  return text.replace(/[\uff00-\uffef]/g, (char) => {
    return (
      fullWidthToHalfWidth[char as keyof typeof fullWidthToHalfWidth] || char
    );
  });
}
