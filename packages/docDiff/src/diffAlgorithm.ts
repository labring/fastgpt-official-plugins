...
export function findExactMatch(
  ...
  for (
    let i = 0;
    i < Math.min(searchRange, modifiedLines.length - startModIndex);
    i++
  ) {
    const line = modifiedLines[startModIndex + i];
    if (line && isExactMatch(originalLine, line)) {
  ...
  for (let i = 0; i <= originalLines.length; i++) {
    ...
    for (let j = 0; j <= modifiedLines.length; j++) {
      if (i === 0 || j === 0) {
        ...
        if (!prevRow || !origLine || !modLine) continue;

        const similarity = calculateSimilarity(origLine, modLine);
        // 中等相似度以上视为潜在匹配
...
export function backtrackLCS(
  ...
  while (i > 0 && j > 0) {
    ...
    if (!origLine || !modLine || !currentRow || !prevRow) break;

    const similarity = calculateSimilarity(origLine, modLine);

    // 中等相似度以上且是匹配路径才视为匹配
    if (
      isMediumSimilarity(similarity) &&
      currentRow[j] === (prevRow[j - 1] ?? 0) + 1
    ) {
      // 找到相似度匹配
...
export function compareDocuments(
  ...
  const { origIndices, modIndices } = backtrackLCS(
    matrix,
    originalLines,
    modifiedLines,
  );
  ...
  for (let matchIndex = 0; matchIndex < origIndices.length; matchIndex++) {
    ...
    if (matchOrigIndex === undefined || matchModIndex === undefined) continue;
