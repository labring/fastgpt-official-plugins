...
export async function handler(
  ...
  // 1. 文本标准化预处理（使用默认配置）
  const normalizedOriginal = applyFullNormalization(
    validatedInput.originalText,
  );
  const normalizedModified = applyFullNormalization(
    validatedInput.modifiedText,
  );
  ...
  const uploadResult = await ctx.emitter.uploadFile({
  ...
  const filteredDiffs = diffs
    .filter(
      (
        diff,
      ): diff is ParagraphDiff & { type: "added" | "removed" | "modified" } =>
        diff.type !== "unchanged",
    )
    .map((diff) => ({
      type: diff.type,
