...
function createTextRun(
  ...
  return new TextRun({
    ...
    color: options.color,
    size:
      options.size && !Number.isNaN(options.size) ? options.size : undefined,
  });
}

function getHeadingLevel(
  level: number,
): (typeof HeadingLevelEnum)[keyof typeof HeadingLevelEnum] {
  switch (level) {
    case 1:
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === "paragraph_open") {
      ...
      if (listStack.length > 0 && isListItemStart) {
        ...
        const prefixText =
          currentList.type === "ordered" ? `${currentList.order}. ` : `• `;
