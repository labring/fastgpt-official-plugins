...
async function processLineWithImages(
  ...
  const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
  let match = imageRegex.exec(remainingText);
  while (match !== null) {
    ...
    if (beforeText) {
      ...
      const rowHeight = style.isTitle
        ? Math.max(lineCount * 20, 25)
        : lineCount * 16;
    ...
    try {
      ...
      worksheet.getRow(currentRow).height =
        Math.ceil(displayHeight / 0.75) + 10;
      currentRow++;
    } catch (error) {
      const cell = worksheet.getCell(currentRow, 1);
    ...
    match = imageRegex.exec(remainingText);
  }
  ...
  for (const block of blocks) {
    ...
    if (isTable) {
      ...
      for (const [rowIdx, rowData] of tableRows.entries()) {
        ...
        for (const [colIdx, cellContent] of rowData.entries()) {
          ...
          if (imageInfo) {
            ...
            const textRowHeight = handleTextCell(
              worksheet,
              text,
              excelRow,
              colIdx,
      ...
      for (const line of lines) {
        currentRow = await processLineWithImages(
          workbook,
          worksheet,
          line,
          currentRow,
...
export async function xlsxTool(
  ...
  try {
    ...
    const finalFilename = filename
      ? `${filename}.xlsx`
      : `markdown-to-excel.xlsx`;
    const result = await ctx.emitter.uploadFile({
      buffer: xlsxBuffer,
    ...
    return Promise.reject(
      `failed to generate excel: ${(error as Error).message}`,
    );
  }
}
